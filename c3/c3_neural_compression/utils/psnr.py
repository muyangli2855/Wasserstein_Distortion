...
if os.path.exists('pmf.p'):
  ...
  def get_pmf(size_list): # x not needed now as input
    ...
    for [H,W] in size_list:
      ...
      H_quarter = int(H/4)
      W_quarter = int(W/4)
      H_3q = int(H - H_quarter)
      W_3q = int(W - W_quarter)
      ...
      pmf_ll = jnp.array([[(1/(2*sigma**2))*jnp.exp(-((i_ref_p-0)**2+(j_ref_p-0)**2)/2*sigma**2)\
              for j_ref_p in range(-W_quarter,W_3q)]\
              for i_ref_p in range(-H_quarter,H_3q)])
      pmf_ll = jnp.expand_dims(pmf_ll/jnp.sum(pmf_ll),axis=2)
      pmf_lc = jnp.array([[(1/(2*sigma**2))*jnp.exp(-((i_ref_p-0)**2+(j_ref_p-0)**2)/2*sigma**2)\
              for j_ref_p in range(-W_half,W_half)]\
              for i_ref_p in range(-H_quarter,H_3q)])
      pmf_lc = jnp.expand_dims(pmf_lc/jnp.sum(pmf_lc),axis=2)
      pmf_lr = jnp.array([[(1/(2*sigma**2))*jnp.exp(-((i_ref_p-0)**2+(j_ref_p-0)**2)/2*sigma**2)\
              for j_ref_p in range(-W_3q,W_quarter)]\
              for i_ref_p in range(-H_quarter,H_3q)])
      pmf_lr = jnp.expand_dims(pmf_lr/jnp.sum(pmf_lr),axis=2)
      pmf_cl = jnp.array([[(1/(2*sigma**2))*jnp.exp(-((i_ref_p-0)**2+(j_ref_p-0)**2)/2*sigma**2)\
              for j_ref_p in range(-W_quarter,W_3q)]\
              for i_ref_p in range(-H_half,H_half)])
      pmf_cl = jnp.expand_dims(pmf_cl/jnp.sum(pmf_cl),axis=2)
      pmf_cr = jnp.array([[(1/(2*sigma**2))*jnp.exp(-((i_ref_p-0)**2+(j_ref_p-0)**2)/2*sigma**2)\
              for j_ref_p in range(-W_3q,W_quarter)]\
              for i_ref_p in range(-H_half,H_half)])
      pmf_cr = jnp.expand_dims(pmf_cr/jnp.sum(pmf_cr),axis=2)
      pmf_ul = jnp.array([[(1/(2*sigma**2))*jnp.exp(-((i_ref_p-0)**2+(j_ref_p-0)**2)/2*sigma**2)\
              for j_ref_p in range(-W_quarter,W_3q)]\
              for i_ref_p in range(-H_3q,H_quarter)])
      pmf_ul = jnp.expand_dims(pmf_ul/jnp.sum(pmf_ul),axis=2)
      pmf_uc = jnp.array([[(1/(2*sigma**2))*jnp.exp(-((i_ref_p-0)**2+(j_ref_p-0)**2)/2*sigma**2)\
              for j_ref_p in range(-W_half,W_half)]\
              for i_ref_p in range(-H_3q,H_quarter)])
      pmf_uc = jnp.expand_dims(pmf_uc/jnp.sum(pmf_uc),axis=2)
      pmf_ur = jnp.array([[(1/(2*sigma**2))*jnp.exp(-((i_ref_p-0)**2+(j_ref_p-0)**2)/2*sigma**2)\
              for j_ref_p in range(-W_3q,W_quarter)]\
              for i_ref_p in range(-H_3q,H_quarter)])
      pmf_ur = jnp.expand_dims(pmf_ur/jnp.sum(pmf_ur),axis=2)
      pmf = jnp.concatenate((pmf_ll,pmf_lc,pmf_lr,pmf_cl,pmf,pmf_cr,pmf_ul,pmf_uc,pmf_ur),axis=2)
