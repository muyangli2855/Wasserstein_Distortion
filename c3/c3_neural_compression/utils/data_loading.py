...
class Textures(data.Dataset):
  ...
  def __init__(
    ...
    self.path_list = [
        os.path.join(self.root, '{:1}.jpg'.format(i))
        for i in range(1, self.num_images + 1)  # Kodak images start at 1
    ]
...
class SingleTexture(data.Dataset):
...
def load_dataset(
  ...
  elif dataset_name.startswith('textures'):
    ds = Textures(root=root, transform=transform)
    start_idx = 0
    end_idx = ds.num_images
  elif dataset_name.startswith('single_texture'):
    ds = SingleTexture(root=root, transform=transform)
    start_idx = 0
    end_idx = ds.num_images
