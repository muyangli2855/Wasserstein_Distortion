...
class Experiment(base.Experiment):
  ...
  def _loss_fn(
    ...
    distortion = psnr_utils.loss_fn(pred_img, target) # mse_fn
    ...
    distortion_rounded = psnr_utils.loss_fn(pred_img_rounded, target) # mse_fn
  ...
  def eval(self, params, inputs, blocked_rates=False):
    ...
    # Compute MSE loss
    distortion = psnr_utils.loss_fn(rec, inputs) # mse_fn
    # Sum rate over all pixels. Ensure that the rate is computed in the space
    # where the bin width of the latents is 1 by passing q_step to the rate
    # function.
    rate = entropy_models.compute_rate(
  ...
  def quantization_step_search(
    ...
    return best_quantized_params, best_metrics,best_rec
  ...
  def step(self, *, global_step, rng, writer):
    ...
    for i, input_dict in enumerate(self._train_data_iterator):
      ...
      _, quantized_metrics,best_rec = self.quantization_step_search(
          params, inputs
      )
      logging.info('Finished quantization step search.')
      best_rec_np = np.array(best_rec)*255
      best_rec_np = best_rec_np.astype(np.uint8)
      best_rec_img = Image.fromarray(best_rec_np)
      best_rec_img.save('recons/{}.png'.format(i+1))
